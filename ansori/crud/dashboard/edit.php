...
<body>
	<div class="container">
		...
		<div class="row">
			<?php
			$id = @$_GET['id'];
			$sql_mhs = mysqli_query($conn, "SELECT * FROM tb_pekerja WHERE id_pekerja = '$id'") or die(mysqli_error($conn));
			...
			<div class="col-lg-6 col-lg-offset-3">
				<form action="proses.php" method="post">
					<div class="form-group">
						...
						<input type="hidden" name="id" name="id" value="<?= $data['id_pekerja'] ?>">
						<input type="text" name="nip" id="nip" autofocus class="form-control" required value="<?= $data['nip'] ?>">
					</div>
					<div class="form-group">
						<label for="nama">Nama pekerja</label>
						<input type="text" name="nama" id="nama"  class="form-control" required required value="<?= $data['nama_pekerja'] ?>">
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<input type="number" name="nik" id="nik" class="form-control" required required value="<?= $data['nik'] ?>">
					</div>
